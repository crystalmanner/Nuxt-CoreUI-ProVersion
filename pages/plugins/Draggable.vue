  ...
  <div class="animated fadeIn">
    ...
    >
      <grid-item
        ...
        drag-ignore-from=".card-body"
        :is-draggable="item.static ? false : true"
      >
        <resize-observer @notify="handleResize(item)" />
        <b-card :id="`coreui-dnd-card-no-${item.i}`" no-body header-tag="header" :class="item.cardClass" class="flex-fill">
          <div slot="header">
            ...
            <div v-if="item.i==='0'" class="card-header-actions">
              <small>
                <b-link variant="link" @click="resetLayout">
                  Reset Layouts
                </b-link>
              </small>
            </div>
...
export default {
  ...
  methods: {
    ...
    async getSize(itemId) {
      const card = await document.getElementById(`coreui-dnd-card-no-${itemId}`)
      const height = parseInt(window.getComputedStyle(card).getPropertyValue('height'), 10)
      const width = parseInt(window.getComputedStyle(card).getPropertyValue('width'), 10)
      return { height, width }
    },
